...
function BLTNotificationsGui:_setup()
	...
	-- Create panels
	self._panel = self._ws:panel():panel({
		w = 400, --profile_panel:w(),
		h = 100 --128
	})
	self._panel:set_center((self._panel:w() * 50) / 100, (self._panel:h() * 75) / 100)
	--self._panel:set_center(1920 / 4, 1280 / 4)
	--self._panel:set_top(self._panel:w() * 2, self._panel:h() * 5)
